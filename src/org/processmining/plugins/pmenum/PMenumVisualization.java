...
public class PMenumVisualization extends JPanel implements Cleanable,
	...
	private JPanel nextPanel, prevPanel;
	...
	private ModelTransitionTable table;
	private ModelTransitionIterator it;
	
	public PMenumVisualization(final ProMJGraph graph,
			final HeuristicsNet net,
			final AnnotatedVisualizationSettings settings) {
		
		init(graph, net, settings);
	}
	
	
	public PMenumVisualization(
			PluginContext context,
			ModelTransitionTable table) {
		
		this.context = context;
		this.table = table;
		this.it = new ModelTransitionIterator(table);
		Double[] thresholds = it.getThis();
		...
		ProMJGraph jgraph = PMenumVisualizer.createJGraph(graph, map, progress);
		
		init(jgraph, net, settings);
	}

	private void init(final ProMJGraph graph,
			final HeuristicsNet net,
			final AnnotatedVisualizationSettings settings) {
		...
		//original
		this.nextPanel = factory.createRoundedPanel(15, Color.DARK_GRAY);
		this.nextPanel.setLayout(null);
		JLabel nextPanelTitle = factory.createLabel("Next");
		nextPanelTitle.setForeground(Color.WHITE);
		nextPanelTitle.setFont(new java.awt.Font("Dialog", java.awt.Font.BOLD,
				18));
		nextPanelTitle.setUI(new VerticalLabelUI(false));
		this.nextPanel.add(nextPanelTitle);
		nextPanelTitle.setBounds(0, 7, 30, 55);

		this.nextPanel.addMouseListener(new MouseListener() {

			public void mouseClicked(MouseEvent e) {
				if(it.hasNext()) {
					Double[] thresholds = it.getNext();
					...
					PMenumVisualization.this.net = FlexibleHeuristicsMinerPlugin.run(context, log, minerSettings, loginfo);
		...
		//original
		this.prevPanel = factory.createRoundedPanel(15, Color.DARK_GRAY);
		this.prevPanel.setLayout(null);
		JLabel prevPanelTitle = factory.createLabel("Prev");
		prevPanelTitle.setForeground(Color.WHITE);
		prevPanelTitle.setFont(new java.awt.Font("Dialog", java.awt.Font.BOLD,
				18));
		prevPanelTitle.setUI(new VerticalLabelUI(false));
		this.prevPanel.add(prevPanelTitle);
		prevPanelTitle.setBounds(0, 7, 30, 55);

		this.prevPanel.addMouseListener(new MouseListener() {

			public void mouseClicked(MouseEvent e) {
				if(it.hasPrev()) {
					Double[] thresholds = it.getPrev();
	...
	private void resize() {
		...
		this.nextPanel.setBounds(width - 30, 40, 40, 75);
		
		this.prevPanel.setBounds(width - 30, 135, 40, 75);
	...
	private void redraw() {
		...
		this.graph = PMenumVisualizer.createJGraph(hng, new ViewSpecificAttributeMap(), null);
